#!/bin/bash
# 阿契 A2A server_v5 启动脚本 —— 碳硅契插件面板「启动服务」/ 自愈 autostart 调用
# slug/port 运行时从 agent.json 读取(身份先行:改名只改 agent.json,这里自动跟随)
set -e
...
if kill -0 "$(cat "$INST_DIR/server.pid")" 2>/dev/null; then
  echo "✅ 阿契 A2A server 已启动 (PID $(cat "$INST_DIR/server.pid"), :$PORT)"
else
